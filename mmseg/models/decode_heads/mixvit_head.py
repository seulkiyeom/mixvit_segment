...
class CatKey(nn.Module):
    def __init__(self, pool_ratio=[1,2,4], dim=[160,64,32]):
        super().__init__()
...
class MixformerHead(BaseDecodeHead):
    """
    Attention-Pooling Former
    """
    def __init__(self, feature_strides, pool_scales=(1, 2, 4), **kwargs):
        super(MixformerHead, self).__init__(input_transform='multiple_select', **kwargs)
        ...
        self.attn_c3 = Block(dim1=c3_in_channels, dim2=tot_channels, num_heads=num_heads[0], mlp_ratio=4,
                                drop_path=0.1, pool_ratio=4)
        self.attn_c2 = Block(dim1=c2_in_channels, dim2=tot_channels, num_heads=num_heads[1], mlp_ratio=4,
                                drop_path=0.1, pool_ratio=2)
        self.attn_c1 = Block(dim1=c1_in_channels, dim2=tot_channels, num_heads=num_heads[2], mlp_ratio=4,
                                drop_path=0.1, pool_ratio=1)

        self.cat_key1 = CatKey(pool_ratio=pool_ratio, dim=[c3_in_channels, c2_in_channels, c1_in_channels])
        self.cat_key2 = CatKey(pool_ratio=pool_ratio, dim=[c3_in_channels, c2_in_channels, c1_in_channels])
        self.cat_key3 = CatKey(pool_ratio=pool_ratio, dim=[c3_in_channels, c2_in_channels, c1_in_channels])
    ...
    def forward(self, inputs):
        x = self._transform_inputs(inputs)  # len=4, 1/4,1/8,1/16
        c1, c2, c3 = x
